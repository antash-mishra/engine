...
const float bias = 0.025; // Reduced bias for more accurate occlusion detection
const float radius = 0.2; // Reduced from 0.2 to make SSAO less aggressive
...
void main() {
    ...
    // Make occlusion even less aggressive for better visual balance
    occlusion = pow(occlusion, power); // Changed from 0.5 to 2.0 to make it less aggressive
    FragColor = occlusion;
}
