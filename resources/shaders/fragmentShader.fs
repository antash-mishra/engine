...
vec3 calcPointLight(vec3 normal, vec3 fragPos, vec3 viewDir) {
    ...
    vec3 ambient = lightColor * texture(tex0, texCoord).rgb;
    vec3 diffuse = lightColor * diff * texture(tex0, texCoord).rgb;
    vec3 specular = lightColor * spec * texture(tex1, texCoord).rgb;
